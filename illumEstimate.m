function imI = illumEstimate(imOri,ps,t0)

[hei,wid,~] = size(imOri) ;
imV = imOri(:,:,1) ;
for i = 1 : 1 : hei
    for j = 1 : 1 : wid
        if imV(i,j) < imOri(i,j,2)
            imV(i,j) = imOri(i,j,2) ;
        end
        if imV(i,j) < imOri(i,j,3)
            imV(i,j) = imOri(i,j,3) ;
        end
    end
end
imBC = vanherk(imV,ps,'max') ;
imBC = vanherk(imBC,ps,'max','col') ;

imDif = imBC - imV ;
A1 = mean(reshape(imDif,1,hei*wid)) ;
imI = min(imV+A1,255) ;
imI = imI / max(reshape(imI,1,hei*wid)) ;
imI = max(imI,t0) ;
